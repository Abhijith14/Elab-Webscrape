...
codedata = []
number = 1
url_no = 1812110#1811110
old_url = 1812110#1811110
for i in range(11,21):
    ...
    if soup.findAll('div', attrs={'class': 'list-item'}):
        ...
        for l in codelist:
            ...
            codestr = codestr + "(br)"
        #print(codestr)
        icon = driver.find_element_by_css_selector('div.CodeMirror-vscrollbar')
        size = icon.size
      #  print(size['height'])
        mainlist.append(codestr)
        mainlist.append(size['height'])
        #print(mainlist)
        ...
        for m in maindata:
            print(m[0])
            print("Size is " + str(m[6]))
            cd = list(m[5].split('(br)'))
            code_string.append(m[5])
            if int(m[6]) > 0:
                code_string = []
                codestring = ""
                print("Data is Large...")
                print("Enter The Code Manually and Close with a <br> tag at the end")
                while True:
                    line = input()
                    if line == "<br>":
                        break
                ...
                multiline_string = "\n".join(code_string)
                for ml in code_string:
                    codestring = codestring + ml
                    codestring = codestring + "(br)"

                #print(codestring)
                #print(len(maindata[0][5]))
                maindata[i][5] = codestring
                #print("----------------------------------")
                maindata[i].pop(6)
                #print(maindata)
                # print(content)
